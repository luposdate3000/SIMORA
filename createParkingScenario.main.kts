...
var dataFile = "src/luposdate3000_simulator_db/src/jvmMain/resources/ontologySOSA.n3"
var defaultQueries = true
...
if (args.size == 0) {
    ...
    println("--size=<NUMBER>")
    println("--data=<FILE>")
    println("--defaultQueries=<BOOLEAN>")
} else {
    for (arg in args) {
        println(arg)
        val a = arg.split("=")
        ...
        val unused = when (k) {
            ...
            "--size" -> size = v.toInt()
            "--data" -> dataFile = v
            "--defaultQueries" -> defaultQueries = v.toBoolean()
    ...
    res.appendLine("                    \"fileName\": \"${dataFile}\",")
